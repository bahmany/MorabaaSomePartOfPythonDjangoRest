/**
 * Created by mohammad on 7/26/15.
 */

var selectedItem;
treeJSON = d3.json("/dms/get-chart-json/", function (error, treeData) {
    ...
    d3.selectAll("#btnPost").on(
        "click", function () {
            $.post("/dms/post-edited-chart-item/", {
                "id": selectedItem.id,
                "name": $("#txtSematName").val(),
                csrfmiddlewaretoken: $.cookie('csrftoken')
            }, function (data) {
                if (data.messageType == "Succ") {
                    showSmallMessageSucc(data.messageHeader, data.messageText);
    ...
    d3.selectAll("#btnAdd").on(
        "click", function () {
            if (!(selectedItem)) {
                smartAlert_Error("خطا", "لطفا یکی از پست های سازمانی را بعنوان پست سازمانی بالایی انتخاب نمایید");
                return;
            }
            var name = window.prompt("plz enter");
            if (name == "") {
                alert("لطفا نام پست سازمانی را وارد نمایید");
                return;
            }
            // second must to open the tree
            $.post("/dms/add-chart-item/", {
                parentID: selectedItem.id,
                name: name,
                csrfmiddlewaretoken: $.cookie('csrftoken')
            }, function (data) {
                addToJson(selectedItem.id, data.messageText, name);
                showSmallMessageSucc("موفقیت", "سمت جدید با موفقیت ثبت شد");
            });
        });
    ...
    d3.selectAll("#btnDelete").on(
        "click", function () {
            if (!(selectedItem)) {
                smartAlert_Error("خطا", "لطفا یکی از پست های سازمانی را انتخاب نمایید");
                return
            }

            $.post("/dms/delete-chart-item/", {
                id: selectedItem.id,
                csrfmiddlewaretoken: $.cookie('csrftoken')
            }, function (data) {
                if (data.messageType == "Succ") {
                    showSmallMessageSucc("موفقیت", data.messageText);
                    removeFromJson(selectedItem.id);
                    return;
                }
                smartAlert_Error("خطا", data.messageText);
                return;
            })
        }
    );
    ...
    function updateParentOfSemat(parentID, sematID) {
        $.post("/dms/change-parent-chart-item/", {
                id: sematID,
                parentId: parentID,
                csrfmiddlewaretoken: $.cookie('csrftoken')
            },
    ...
    function click(d) {
        ...
        var scope = angular.element($("#divPermissionChart")).scope();
        scope.GetAccessJson(d.id);
